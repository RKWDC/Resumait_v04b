...
export const scoreResumePipelined = ai.defineFlow(
  ...
  async (input) => {
    ...
    // Extract job title from JD text (first line usually)
    ...
    const result = {
      ...
      qualitativeRating: masterMatch.breakdown.hardSkillsMatch >= 75 ? 'Excellent' : masterMatch.breakdown.hardSkillsMatch >= 60 ? 'Strong' : 'Moderate',
      scoreBreakdown: {
        hardSkillsCertifications: { earned: masterMatch.breakdown.hardSkillsMatch, max: 80 },
