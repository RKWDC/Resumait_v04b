  ...
  async (input) => {
    ...
    try {
      const { text } = await ai.generate({
        config: { temperature: 0.2, maxOutputTokens: DEFAULT_MAX_OUTPUT_TOKENS, timeout: 110000 },
        system: `You are a phrase-level NLP extraction engine. Return ONLY valid JSON.
        
        {
          ...
          "extractedPhrases": [
            {
              "surfaceTerm": "string",
              "requirementTier": "required or preferred",
              "priorityWeight": 1,
        ...
        prompt: `Extract keywords from this JD: ${input.jobDescription}`,
      });
      task1Result = Task1OutputSchema.parse(cleanJsonResponse(text));
    } catch (e) {
    ...
    try {
      const { text } = await ai.generate({
        ...
        system: `Normalize and classify these phrases. Return ONLY valid JSON.
          ...
          "enrichedKeywords": [
            {
              ...
              "category": "hard_skill or soft_skill or certification or tool_platform",
              "requirementTier": "required or preferred",
              "priorityWeight": 1,
              "context": "string"
            }
        ...
        prompt: `Phrases: ${JSON.stringify(task1Result.extractedPhrases)}`,
      });
      task2Result = Task2OutputSchema.parse(cleanJsonResponse(text));
    } catch (e) {
      task2Result = {
    ...
    // 4. TASK 3: Implied Skills
    let task3Result;
    try {
      const { text } = await ai.generate({
        config: { temperature: 0.2, maxOutputTokens: DEFAULT_MAX_OUTPUT_TOKENS, timeout: 110000 },
        system: `Infer implied skills. Return ONLY valid JSON.
        ...
        prompt: `JD: ${input.jobDescription}\nTitle: ${task1Result.jobTitle}\nKeywords: ${JSON.stringify(task2Result.enrichedKeywords.map(k => k.canonicalTerm))}`,
      });
      task3Result = Task3OutputSchema.parse(cleanJsonResponse(text));
    } catch (e) {
      task3Result = {
        impliedKeywords: [],
